  ...
  return (
    <div className="relative w-28 h-28 shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-[9px] text-slate-500 font-medium">ISOLATION</span>
      </div>
    </div>
  );
}
...
function DistrictDetail({ district }: { district: District }) {
  ...
  return (
    <div className="h-full overflow-y-auto p-6 animate-fade-up">
      <div className="mb-5">
        <h3 className="text-white font-bold text-base mb-1">{district.name}</h3>
        <span className={clsx(
          'text-xs font-bold px-2 py-1 rounded',
          district.status === 'ISOLATED' ? 'bg-red-500/20 text-red-400 border border-red-500/30' :
          district.status === 'HIGH_RISK' ? 'bg-orange-500/15 text-orange-400 border border-orange-500/20' :
          district.status === 'DEGRADED' ? 'bg-yellow-500/15 text-yellow-400 border border-yellow-500/20' :
          district.status === 'ACCESSIBLE' ? 'bg-green-500/15 text-green-400 border border-green-500/20' :
      ...
      <div className="glass-card p-4 rounded-xl flex items-center gap-6 mb-4">
        ...
        <div className="flex-1 space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="bg-white/[0.04] rounded-lg px-2.5 py-2">
              <div className="text-slate-500 text-[10px]">Alt. Routes</div>
              <div className={clsx('font-bold', district.alternativeRoutes === 0 ? 'text-red-400' : district.alternativeRoutes <= 1 ? 'text-orange-400' : 'text-green-400')}>
                {district.alternativeRoutes}
              </div>
            </div>
            <div className="bg-white/[0.04] rounded-lg px-2.5 py-2">
              <div className="text-slate-500 text-[10px]">Critical Corridors</div>
              <div className="text-white font-bold">{district.criticalCorridors}</div>
            </div>
            <div className="bg-white/[0.04] rounded-lg px-2.5 py-2">
              <div className="text-slate-500 text-[10px]">Weather Risk</div>
              <div className={clsx('font-bold', weatherColors[district.weatherRisk])}>{district.weatherRisk}</div>
            </div>
            <div className="bg-white/[0.04] rounded-lg px-2.5 py-2">
              <div className="text-slate-500 text-[10px]">Supply Coverage</div>
              <div className={clsx('font-bold', district.supplyDays <= 2 ? 'text-red-400' : district.supplyDays <= 4 ? 'text-orange-400' : 'text-green-400')}>
      ...
      {criticalRoad && (
        <div className="glass-card p-4 rounded-xl mb-4 border-orange-500/15">
          <div className="text-xs font-bold text-slate-300 mb-3">
            🔮 IF <span className="text-orange-400">{criticalRoad.name.split('—')[0].trim()}</span> FAILS:
          </div>
          <div className="space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-slate-400">District connectivity</span>
              <span className="text-red-400 font-bold">→ ~41%</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Isolation probability</span>
              <span className="text-red-400 font-bold">→ {district.isolationRisk}%</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-orange-400 font-bold">MEDIUM RISK</span>
            </div>
          </div>
      ...
      {distInventory.length > 0 && (
        <div className="glass-card p-4 rounded-xl">
          <div className="text-xs font-bold text-slate-300 mb-3">📦 Commodity Stock</div>
          <div className="space-y-3">
            {distInventory.map((inv) => (
              <div key={inv.commodity}>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-slate-400 capitalize">{inv.commodity === 'agri' ? 'Agricultural' : inv.commodity}</span>
                  <span className={clsx(
                    'font-bold',
                    inv.risk === 'CRITICAL' ? 'text-red-400' :
                    inv.risk === 'HIGH' ? 'text-orange-400' :
                    inv.risk === 'MEDIUM' ? 'text-yellow-400' : 'text-green-400'
    ...
    <div className="flex h-full min-h-0">
      ...
      <div className="w-72 shrink-0 bg-[#090f1c] border-r border-white/[0.06] flex flex-col">
        <div className="px-4 py-3 border-b border-white/[0.06] flex items-center justify-between">
          <span className="text-white text-sm font-semibold">Districts</span>
            ...
            return (
              <button
                ...
                className={clsx(
                  ...
                  selected?.id === d.id && 'bg-white/[0.06] border-l-2 border-l-orange-500'
                )}
              >
                <div className="relative w-12 h-12 shrink-0">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-[9px] font-bold" style={{ color: riskColor }}>
                    {d.isolationRisk}%
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-white text-xs font-semibold truncate">{d.name.split('—')[0].trim()}</div>
                  <div className="text-slate-500 text-[10px] mt-0.5">{d.name.split('—')[1]?.trim()}</div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px]" style={{ color: riskColor }}>{d.status.replace('_', ' ')}</span>
                    <span className="text-slate-600 text-[10px]">·</span>
                    <span className="text-slate-500 text-[10px]">{d.supplyDays}d supply</span>
                  </div>
                </div>
                <ChevronRight size={14} className={clsx('text-slate-600 shrink-0', selected?.id === d.id && 'text-orange-400')} />
              </button>
            );
          })}
        </div>
      ...
      <div className="flex-1 min-w-0 bg-[#0c1424]">
        {selected ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-600">
            ...
            <p className="text-sm">Select a district to view isolation intelligence</p>
          </div>
        )}
      </div>
    </div>
